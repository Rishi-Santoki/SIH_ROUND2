...
export function ProofBadge({ status, className, showLabel = false }: ProofBadgeProps) {
  if (status === 'verified') {
    return (
      <div className={cn("inline-flex items-center gap-1.5", className)}>
        ...
        {showLabel && <span className="text-sm font-medium text-ink">Verified</span>}
      </div>
    );
  }
  ...
  if (status === 'pending') {
    return (
      <div className={cn("inline-flex items-center gap-1.5", className)}>
        ...
        {showLabel && <span className="text-sm font-medium text-slate">Verification Pending</span>}
      </div>
    );
  }
